...
export const SignUpForm = () => {
  ...
  const { refetch: checkEmailDuplicate } = useCheckEmailDuplicate(watch("email"));
  const { mutate: signUp, data } = useSignUp({
    email: watch("email"),
    password: watch("password"),
  });
  const validateAlreadyExist = async () => {
    const response = await checkEmailDuplicate();
    const isUsableEmail = response?.data?.data?.isUsableEmail;
    const errorResponseStatus = response?.error?.response?.status;
    if (errorResponseStatus === 409) {
      ...
      <div className={cx("input-box")}>
        ...
        <Controller
          ...
          rules={{
            ...
            validate: {
              alreadyExist: validateAlreadyExist,
            },
          }}
