...
const FolderPage = () => {
  ...
  return (
    <Layout isSticky={false} footerRef={ref}>
      <FolderLayout
        linkForm={<LinkForm hideFixedLinkForm={isIntersecting} currentFolderId={currentFolderId} />}
        searchBar={
          <SearchBar value={searchValue} onChange={handleChange} onCloseClick={handleCloseClick} />
        }
        folderToolBar={<FolderToolBar folders={folders} currentFolderId={currentFolderId} />}
        cardList={isLoading ? null : <CardList links={result} currentFolderId={currentFolderId} />}
      />
    </Layout>
  );
};
